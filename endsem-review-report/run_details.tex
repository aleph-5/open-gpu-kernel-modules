...
\mainsubsubsection{Plotting Benchmark Results}

The plotting script \texttt{plot\_overhead.py} and requires the csv file
\texttt{overhead\_results.csv} as input.

The script requires \texttt{matplotlib} and \texttt{numpy}:
...
Each figure plots overhead percentage against working-set size (512\,MB --
64\,GB).  A shaded region marks the oversubscribed regime (working set exceeds
GPU memory).

\mainsubsubsection{Backend Comparison}

Recompile once per backend (set \texttt{.ops} in \texttt{uvm\_common.c}, then
\texttt{sudo ./compile.sh}), then collect stats and plot:
...
\mainsubsubsection{Query-Frequency Sensitivity}
