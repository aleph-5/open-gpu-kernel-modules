...
\subsection*{Milestone 2 - Extensions, Testing and Optimisations \hfill \textit{(4 weeks)}}
    \begin{enumerate}
        ...
        \begin{enumerate}
            ...
            CPU and GPU side, and removing write permission for the tracked pages at init, instead of invalidating them to reduce overread
             due to read-only GPU pages and using kernel threads to parallelly do the same to achieve speedup % range-based dirty tracking to allow selective monitoring of specific virtual memory regions. %%ARUSH: Arn't we doing this already? replacing with a new point
