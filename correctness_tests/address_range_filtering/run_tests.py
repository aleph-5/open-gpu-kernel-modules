#!/usr/bin/env python3
"""
run_tests.py — address_range_filtering test runner
...
Usage:
    ...
    sudo python3 run_tests.py tc01 tc02    # run specific tests by prefix
"""
...
def main():
    parser = argparse.ArgumentParser(description="address_range_filtering test runner")
    parser.add_argument("tests", nargs="*", help="test prefixes to run (e.g. tc01 tc02)")
    ...
    print(color(f"  address_range_filtering — {len(binaries)} test(s)", ANSI_BOLD))
    print(color(f"{'='*60}", ANSI_BOLD))
    print()
