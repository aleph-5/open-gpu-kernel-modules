...
/*
 * Scan BFS (algorithm 3):
 * For each u in currentQueue, check each neighbour v of u
 * Set d_distance[v] = d_distance[u] + 1 if not visited.
 * This is called ONLY for vertices at a given level/distance.
 */
__global__
...
/*
 * For each u in the current layer (distance == level), for each neighbour
 *   v of u, do degree[u]++ if v was visited in the last iteration.
 * Misleading name: degree() is the number of vertices introduced by u, which
 *   will be processed in the next iteration. This kernel finds individual degrees.
 *   Could have been a part of nextLayer() too.
 */
__global__
...
/*
 * Use recursive summation to find the sum of the current thread block's
 * degrees. Write the sum to incrDegrees[], and undo the recursive summing
