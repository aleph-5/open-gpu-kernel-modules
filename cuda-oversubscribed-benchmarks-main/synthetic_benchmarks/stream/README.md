...
**NOTE:** This is a stripped-down copy of the original benchmark, which supports
CUDA only.   
`CUDAStream.cu` was `#include`d in `main.cu` to share the global table of CUDA
hints, from `cuda-macros-v1.h`.   
A regular `make` should correctly build the binary.
