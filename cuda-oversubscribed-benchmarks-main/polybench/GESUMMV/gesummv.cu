...
__global__ void gesummv_kernel(DATA_TYPE *a, DATA_TYPE *b, DATA_TYPE *x, DATA_TYPE *y, DATA_TYPE *tmp, long N)
{
...
void gesummvCuda(DATA_TYPE* A_gpu, DATA_TYPE* B_gpu, DATA_TYPE* x_gpu, DATA_TYPE* y_gpu, DATA_TYPE* tmp_gpu)
{
	...
	t_start = rtclock();
	for (int i = 0; i < 1; i++){
	gesummv_kernel<<< grid, block, 0 , stream5>>>(A_gpu,B_gpu,x_gpu, y_gpu, tmp_gpu, N);
...
int main(int argc, char *argv[])
{
	...
	N = get_data_size(data_size/2, sizeof(DATA_TYPE), 2);
