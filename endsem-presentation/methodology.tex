...
\begin{frame}{Design Choices}{Query Modes (Cummulative)}
\begin{itemize}[<+->]
    \item \textbf{Cummulative Mode} dumps the pages written to since the start of tracking
...
\begin{itemize}[<+->]
    \item \texttt{ds\_rwsem} -- shared on fault-handler inserts/lookups and
    \texttt{query\_dump}; exclusive only for init/destroy and the cutover
    pointer swap
    \item \texttt{lifecycle\_lock} -- mutex serialising all procfs lifecycle
    operations
    \item \texttt{va\_spaces.lock} + \texttt{va\_space->lock} -- held across
    the downgrade walk to close the active-before-downgrade and
    downgrade-before-activate races
    \item \texttt{va\_block->lock} + per-GPU ISR lock -- query barrier locks
    all replayable-fault ISRs and drains the fault buffer for a precise
    cutover/pause boundary
    \item \texttt{WRITE\_ONCE}/\texttt{READ\_ONCE} on the tracking-active
    flag; backend-level locks (xarray \texttt{xa\_cmpxchg}, unsorted-vector
    IRQ spinlock)
    \item \textbf{Lock order}: \texttt{lifecycle\_lock} $\to$
    \texttt{va\_spaces.lock} $\to$ \texttt{va\_space.lock} $\to$ ISR $\to$
    \texttt{va\_block.lock} $\to$ \texttt{ds\_rwsem}
\end{itemize}
\end{frame}
...
\begin{itemize}[<+->]
    ...
    \end{itemize}
    \item An abstraction layer for data structure is defined by \texttt{uvm\_dirty\_ds\_ops}, 
    which declares a common interface for insert, lookup, for-each, init, destroy, and search 
    operations.
    \item Each of the operation interfaces has a tracker that can track number of operations
    and total time taken by them when stats are enabled 
\end{itemize}
\end{frame}
