...
\node[desc, right=16mm of vas] {Mutex; Held across the global VA space walk to find teh owner process};
\node[desc, right=16mm of isr] {Semaphore; Start/resume: held during PTE downgrade + active=true setting. Pause/cutover: guarantees all in-flight faults are ds_insert-ed before the epoch boundary is sealed};
\node[desc, right=16mm of va]  {rwsem; Protects the VA range tree structure};
...
\node[desc, right=16mm of vab] {Mutex; Held per VA block during PTE downgrade/restore};
\node[desc, right=16mm of rw]  {rwsem; Shared for fault-handler inserts/lookups and \texttt{query\_dump}. Exclusive for init/destroy and the cutover pointer swap};
