...
\node[lock, below=of vas] (isr) {per-GPU ISR lock};
\node[lock, below=of isr] (va)  {va\_space.lock};
\node[lock, below=of va]  (vab) {va\_block.lock};
\node[lock, below=of vab] (rw)  {ds\_rwsem};

\node[desc, right=16mm of lc]  {Mutex; serialises \emph{all} procfs lifecycle operations};
\node[desc, right=16mm of vas] {Mutex; held across the global VA space walk to find the owner process};
\node[desc, right=16mm of isr] {Semaphore; start/resume: held during PTE downgrade + \texttt{active=true} setting. Pause/cutover: guarantees all in-flight faults are \texttt{ds\_insert}-ed before the epoch boundary is sealed};
\node[desc, right=16mm of va]  {rwsem; protects the VA range tree structure; write lock blocks fault handlers during downgrade, read lock during restore};
\node[desc, right=16mm of vab] {Mutex; held per VA block during PTE downgrade/restore and fault servicing};
